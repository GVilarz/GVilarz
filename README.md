<div align="center">

  <a href="https://git.io/typing-svg">
    <img src="https://readme-typing-svg.demolab.com?font=Fira+Code&weight=600&size=30&pause=1000&color=2563EB&center=true&vCenter=true&width=500&lines=Gabriel+Vilar;Desenvolvedor+Web;Bem-vindo+ao+meu+perfil!" alt="Typing SVG" />
  </a>
</p>
  
  <p align="center">
  <a href="https://git.io/typing-svg">
    <img src="https://readme-typing-svg.demolab.com?font=Fira+Code&weight=600&size=30&pause=1000&color=9333EA&center=true&vCenter=true&width=500&lines=Estudante+em+Tecnologia;Desenvolvedor+em+evolução;Sempre+aprendendo" alt="Typing SVG" />
  </a>
</p>


<a href="https://gvilarz.github.io/Link-Bio/">
    <img src="https://skillicons.dev/icons?i=linkedin,instagram,gmail,spotify,github" />
  </a>
</p>
</div>

---

### 👤 Sobre Mim

- 🔭 **Projetos Atuais:** Trabalhando em `Tecnologia`.
- 🌱 **Evoluindo em:** `Banco de dados (MySQL) e FrontEnd e BackEnd`.
- ⚡ **Diferencial:** `Sempre apaixonado por tecnologia`.
- 📍 **Localização:** `Guarulhos/São Paulo`.

---

### 💻 Tech Stack (Badges Personalizáveis)


**Linguagens & Frameworks:**
[![](https://skillicons.dev/icons?i=python,html,css,c)](https://skillicons.dev)

**Banco de Dados & Infra:**
[![MySQL](https://skillicons.dev/icons?i=mysql)](https://skillicons.dev)
[![Git](https://skillicons.dev/icons?i=git)](https://skillicons.dev)

---

### 📌 Projetos em Destaque (Pins Personalizados)

| Projeto | Descrição | Techs | Link |
| :--- | :--- | :--- | :---: |
| **🚀 Projeto 1** | Criando Link na BIO. | `HTML` `CSS` | [Acessar 🔗](https://github.com/GVilarz/Link-Bio) |
| **📦 Projeto 2** | Pesquisa Sobre FrontEnd e BackEnd. | `JavaScript` `CSS` | [Acessar 🔗](https://github.com/GVilarz/pesquisa-front.back) |
| **📱 Projeto 3** | Todas nossas aulas . | `HTML` `CSS` | [Acessar 🔗](https://github.com/GVilarz/LIMA) |

---

### 📊 Estatísticas e Atividade

<div align="center">

  <img height="165em" src="https://github-readme-stats.vercel.app/api?username=GVilarz&show_icons=true&theme=dracula&include_all_commits=true&count_private=true"/>
  <img height="165em" src="https://github-readme-stats.vercel.app/api/top-langs/?username=GVilarz&layout=compact&theme=dracula&hide=html,css"/>

</div>

<br />

<div align="center">
  <img src="https://profile-counter.glitch.me/GVilarz/count.svg" alt="Contador de Visitas" />
</div>
